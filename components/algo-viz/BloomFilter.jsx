...
export default function BloomFilter() {
  ...
  return (
    <>
      <div class="bftContainer" id="bft-container" ref={svg} />
    </>
  );
}


function BFT_vis(svgRef) {
  ...
  svgRef.current.appendChild(btnDiv1);
  ...
  function changeSize() {
    ...
    if (newSize < 2 || newSize % 1 !== 0 || newSize > 50){
      ...
      // change canvas size
      let desHeight =  600 + Math.max(0, newSize-18)*28;
  ...
  function insertValue() {
    ...
    if (dup || newInsert.length > 20 || newInsert ==''){
      ...
      // change canvas size
      let desHeight =  600 + Math.max(0, BFT.arrayOfElems.length-12)*28;
      let desViewBox = "0 0 900 " + desHeight;
      document.getElementById("bft-canvas").setAttribute("viewBox", desViewBox);
  ...
  function drawArrow(bft) {
    ...
    for (var j = 0; j < 4; j++) {
      ...
      let link = d3.linkHorizontal()
          ...
          .target(function (d,i) {
            let idx = bft.hash(d)[j];
            let y = getDataY(d, idx);
            return [250, y+10]
          });
  ...
  function drawFind(find) {
    ...
    switch (found) {
      case "Positive":
        ...
        message3 = "In this scenario, it is a True Positive."
        break;
      case "False Positive":
        message1 = "Because all of the hashed position is occupied,";
        message2 = "the searched value is probably there.";
        message3 = "In this scenario, it is a False Positive.";
        break;
