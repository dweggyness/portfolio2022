...
export default function EHTHash() {
  const svg = useRef();
  ...
  return (
    <>
      <div class="ehtContainer" id="eht-container" ref={svg} />
    </>
  );
}


function EHT_vis(svgRef) {
  ...
  class ExtendibleHashingTable {
    ...
    */
    splitBucket(key) {
      ...
      console.log(key, this.directories);
      console.log(this.directories[key]);
      console.log(bucket);
    ...
    */
    insertIntoTable(data) {
      ...
      if (bucket.isFull()) {
        ...
        console.log("HELLO line188");
        let b = this.splitBucket(key);
        let b1Key = b[0];
        let b1 = b[1];
        let b2Key = b[2];
        let b2 = b[3];
        let splitAgain = this.splitAgain(b1Key, b1, b2Key, b2);
      }
    }
  ...
  function iniEHT() {
    ...
    if (newIniNum < 0 || newIniNum % 1 !== 0){
      ...
      // change canvas size
      let desHeight =  600 + Math.max(0, newIniNum-18)*28;
      let desViewBox = "0 0 900 " + desHeight;
      ...
      for (var i = 0; i< NUM_ELEMS_TO_INSERT; i++ ) {
        ...
        let hashedKey = EHT.hash(val);
        console.log('hashedkey', hashedKey)
        console.log('EHT.globaldealth', EHT.globalDepth)
        ...
        console.log(convKey, "420");
        if (bucket.isFull()) {
          convKey = hashedKey & ((1 << EHT.globalDepth) - 1)
          console.log(convKey, hashedKey, "422 fr");
          if (bucket.localDepth == EHT.globalDepth) {
            console.log(convKey, "hello change");
            EHT.growDirectories();
          }

          // EHT.splitBucket(convKey);
          console.log(convKey, "HELLO 422");
          let b = EHT.splitBucket(convKey);
          console.log('goodbye 422')
          ...
          // update EHT record
          let tempEHT = new ExtendibleHashingTable();
          EHT.deepcopyto(tempEHT);
          ehtRecord.push(tempEHT);

          let convKey = hashedKey & ((1 << EHT.globalDepth) - 1);

          let b1Key = b[0];
          let b1 = b[1];
          let b2Key = b[2];
          let b2 = b[3];
          let splitAgain = EHT.splitAgain(b1Key, b1, b2Key, b2);
  ...
  function drawKeyArrow(eht) {
    ...
    // buckets & values
    let result = getUniqueBuckets(bucketData);
    let uniqueBuckets = result[0];
    let bucketIndices = result[1];
  ...
  function drawValues(eht) {
    ...
    let flattedData = flatOutBuckets(uniqueBuckets);
  ...
  function insertValue() {
    ...
    if (newInsert < 0 || newInsert % 1 !== 0 || dup){
      ...
      // change canvas size
      let desHeight =  600 + Math.max(0, insertIdx-18)*28;
      let desViewBox = "0 0 900 " + desHeight;
      document.getElementById("eht-canvas").setAttribute("viewBox", desViewBox);

      stateHash.push(true);

      let hashedKey = EHT.hash(newInsert);
